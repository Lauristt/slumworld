...
TILE_CONFIG_LIST=${1:-"~/slumworldML/config/tile_config_kfold_list.lst"}

if [[ $TILE_CONFIG_LIST == "h" ]] || [[ $TILE_CONFIG_LIST == "help" ]]; then
    echo "Script to use for scheduling k-fold tiling raw images (using a different tiling specification for each one) in parallel."
    ...
    echo "              it is reccomended to place this within \"\" [default:\"~/slumworldML/config/tile_config_kfold_list.lst\"]"
    echo "USAGE:"
    echo "   To train a model using the a different tile_config_kfold_list1.lst"
    echo "   . schedule-tile_kfold.sh \"\/path\/to\/tile_config_kfold_list2.lst"
    echo ""
    echo "   To display usage help:"
    echo "   . schedule-tile_kfold.sh  h"
    echo ""
...
chmod +x ~/slumworldML/slurm/tile_kfold.slurm
