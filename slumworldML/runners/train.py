...
def model_trainer(cl_vars):
    ...
    if not cl_vars['from_config']:
        # update config variables with command line arguments if not None
    ...
    if conf['training_parameters']['scheduler']['name'] in ['Exponential', 'OneCycleLR']:
        callbacks.append(LearningRateMonitor(logging_interval="step"))
    elif conf['training_parameters']['scheduler']['name'] in ['Plateau', 'CosineWarmRestarts', 'MultistepLR' ]:
        callbacks.append(LearningRateMonitor(logging_interval="epoch"))
    if conf['training_parameters']['stochastic_weight_averaging']:
        pass
        callbacks.append(StochasticWeightAveraging(swa_epoch_start=0.8, annealing_strategy='cos', 
                                                   annealing_epochs=math.floor(0.2*conf['training_parameters']['num_epochs']),swa_lrs=conf['training_parameters']['learning_rate'] / 10
                                                   )
                        )
    ...
    if conf['training_parameters']['scheduler']['name'] == 'OneCycleLR':
    ...
    model =  ModelTrainingWrapper(training_params=conf['training_parameters'], 
                                  ...
                                  from_pretrained=conf['run_type']['pretrained_model'],
    ...
    if conf['run_type']['from_checkpoint'] or cl_vars['checkpoint_file']:
        if conf['paths']['checkpoint_file'] is not None:
            if os.path.exists(conf['paths']['checkpoint_file']) and os.path.getsize(conf['paths']['checkpoint_file']) > 0:
                resume_from_checkpoint = conf['paths']['checkpoint_file']
                print('-'*100)
                print("Training resumed from saved checkpoint file:\n",resume_from_checkpoint)
                print('-'*100)
            else:
