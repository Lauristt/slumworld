...
class Predictor():
    ...
    def _load_model(self):
        ...
        try:
            ...
            # remove binary classifier weightss so that you can load the standard model
            state_dict = {k:v for k,v in state_dict.items() if not k.startswith('binary_classifier')}
            self.model.load_state_dict(state_dict)
        except RuntimeError: 
            try:
                new_state_dict = torch.load(self.model_path, map_location=torch.device(self.device))['state_dict']
                # saved keys include the string 'model.' at the begining of each weight - pytorch_lightning
                new_state_dict = {k.replace('model.',''):v for k,v in new_state_dict.items() if k != "criterion.pos_weight"}
                new_state_dict = {k:v for k,v in new_state_dict.items() if not k.startswith('binary_classifier')}
                self.model.load_state_dict(new_state_dict)
                self.model.to(self.device)
            except RuntimeError:
                new_state_dict = torch.load(self.model_path, map_location=torch.device(self.device))['state_dict']
                # saved keys include the string 'model.' at the begining of each weight - pytorch_lightning
                new_state_dict = {k.replace('model.model.','model.'):v for k,v in new_state_dict.items() if k != "criterion.pos_weight"}
                new_state_dict = {k:v for k,v in new_state_dict.items() if not k.startswith('binary_classifier')}
                self.model.load_state_dict(new_state_dict)
                self.model.to(self.device)
    ...
    def predict_for_batches(self, number_of_batches=1, visualize=True, return_labels=True, return_names=True):
        """
        Makes predictions on batches of data and plots the results.

        :param number_of_batches: Number of batches to make predictions on.
        """
        ...
        with torch.no_grad():
            for i in range(number_of_batches):
                haslabels = hasnames = False
                dat  = next(data_iter)
                if len(dat) == 3:
                    images, labels, names = dat[0], dat[1], dat[2]
                    haslabels = hasnames = True
                elif len(dat) == 2:
                    images, names = dat[0], dat[1]
                    hasnames = True
                else:
                    images = dat[0]
                outputs = self._batch_predict(images.to(self.device))
                # outputs_list.append(outputs)
                outputs_list.append(copy.deepcopy(outputs))
                if visualize and haslabels:
                ...
                if return_labels and haslabels:
                    try:
                        labels_list.append(copy.deepcopy(labels.detach().cpu().numpy()))
                    except AttributeError as Error:
                        labels_list.append(copy.deepcopy(labels))

                if return_names and hasnames: 
                    tile_names.append(copy.deepcopy(names))
                    # tile_names.append(copy.deepcopy(names).detach().cpu().numpy())

            if haslabels and hasnames:
    ...
    def evaluate_model_ensemble(self, models_list=None, model_checkpoints=None, strategy='majority',
        ...
        for model_i, checkpoint_i in zip(models_list, model_checkpoints):
            print(f"\nPredicting for model {i+1}, type: {model_i}")
            ...
            outputs_list_i, self.labels_list, self.tile_names = self.predict_with_tta_loop(num_augs=num_augs, USE_TTA=TTA)
            if release_memory:
        ...
        print("\nPredictions completed.")
        self._print_save_return_metrics(save_tiles=save_tiles, output_path=output_path, filename=filename, 
                                        predict_only=predict_only, dataset_csv=dataset_csv)
