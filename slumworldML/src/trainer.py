...
class ModelTrainingWrapper(LightningModule):
    ...
    def __init__(self, model= None, model_params=None, training_params=None, domain_loss_scaling_factor=6e+4,
        ...
        self.hparams['model'] = MODEL_PARAMS
        if (model is None) and (from_pretrained is None):
            if model_params is not None:
                self.model = UNet(domain_classifier=domain_classifier, **model_params)
                self.set_model_parameters(update_model=False, **model_params)
            else:
                print("MODEL_PARAMS",MODEL_PARAMS)
                self.model = UNet(**MODEL_PARAMS)
                self.set_model_parameters(update_model=False, **MODEL_PARAMS)
        elif (model is None) and (from_pretrained):
            if model_params is not None:
                self.model = MODELS_REGISTRY[from_pretrained](model_params['in_channels'], model_params['out_channels'], 
                                                              train_head_only=training_mode=='freeze', domain_classifier=domain_classifier)
            else:
                self.model = MODELS_REGISTRY[from_pretrained](MODEL_PARAMS['in_channels'], MODEL_PARAMS['out_channels'], 
                                                              train_head_only=training_mode=='freeze', domain_classifier=domain_classifier)
        else:
        ...
        if training_params is not None:
            self.set_training_parameters(**training_params)
        else:
            self.set_training_parameters(**TRAINING_PARAMS)
    ...
    def set_optimizer(self, optimizer):
        ...
        if optimizer in ['Adam', 'AdamW']:
           self.optimizer = self.SUPPORTED_OPTIMIZERS[optimizer](self.model.parameters(), lr=self.learning_rate, weight_decay=self.l2_reg)
        elif optimizer in ['SGD']:
            self.optimizer = self.SUPPORTED_OPTIMIZERS[optimizer](self.model.parameters(), lr=self.learning_rate, momentum=self.momentum, weight_decay=self.l2_reg)
        elif optimizer in ['AdaBound', 'AdaBoundW']:
            self.optimizer = self.SUPPORTED_OPTIMIZERS[optimizer](self.model.parameters(), lr=self.learning_rate, final_lr=self.final_lr, weight_decay=self.l2_reg)
        elif optimizer == 'Yogi':
            self.optimizer = self.SUPPORTED_OPTIMIZERS[optimizer](self.model.parameters(), lr=self.learning_rate, weight_decay=self.l2_reg)
    ...
    def forward(self, x):
        # here you an add logic for different models (e.g. DeepLab etc) that might output dicts, etc
        output = self.model(x)
    ...
    def _calc_metrics(self, labels, outputs):
        if labels.min().cpu() > 0.0:
            hard_labels = (labels > self.threshold).long()
        else:
            hard_labels = labels
        preds = (torch.sigmoid(outputs) > self.threshold).float()
        acc = binary_accuracy(preds=preds, target=hard_labels)#, threshold=self.threshold, num_classes=1)
        f1_score = binary_fbeta_score(preds=preds, target=hard_labels, beta=1.0)#, threshold=self.threshold, num_classes=1)
        iou_score = binary_jaccard_index(preds=preds, target=hard_labels)#, threshold=self.threshold, num_classes=1)
    ...
    def on_train_start(self):
        ...
        if hasattr(self, 'trainer') and self.trainer is not None and self.trainer.loggers:
            ...
            for logger_instance in loggers:
                # 使用 from pytorch_lightning.loggers import CSVLogger, TensorBoardLogger
    ...
    def training_step(self, batch, batch_idx):
        inputs, labels, paths = batch['slum_prediction']
        labels = labels.to(dtype=inputs.dtype)
        batch_size_1 = len(paths)
        
        if self.classifier:
            images, names = batch['domain_prediction']
            batch_size_2 = len(images)
            domain_labels = torch.cat([torch.zeros([batch_size_1, 1]), torch.ones([batch_size_2, 1])])
            domain_labels = domain_labels.to(dtype=images.dtype).to(self.device)
            inputs_ = torch.cat((inputs, images), axis=0)
            outputs_, logits = self.model(inputs_)
        else: 
            inputs_ = inputs
    ...
    def validation_step(self, batch, batch_idx):
        print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step called for batch_idx: {batch_idx}.")
        if isinstance(batch, (list, tuple)) and len(batch) == 1 and isinstance(batch[0], (list, tuple)):
            print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step: Unpacking single-element list/tuple batch wrapper.")
            batch = batch[0] # unpackage
        inputs, labels, paths = None, None, None 
        # robust unpackaging, for debugging and original usage
        if isinstance(batch, dict) and 'slum_prediction' in batch:
            inputs, labels, paths = batch['slum_prediction']
            print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step: Batch unpacked as DICT['slum_prediction'].")
        elif isinstance(batch, (list, tuple)) and len(batch) == 3: 
            inputs, labels, paths = batch
            print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step: Batch unpacked as direct TUPLE.")
        else:
            print(f"ERROR_MTW: Epoch {self.current_epoch} - validation_step: Unexpected batch format! Type: {type(batch)}")
            if isinstance(batch, (list, tuple)):
                print(f"ERROR_MTW: Batch content len: {len(batch)}")
                if len(batch) > 0:
                    print(f"ERROR_MTW: First element type: {type(batch[0])}")
                    if hasattr(batch[0], 'shape'):
                        print(f"ERROR_MTW: First element shape: {batch[0].shape}")
            raise ValueError(f"Unsupported batch format in validation_step. Received type: {type(batch)}")
        # ========================================
        if batch_idx == 0: 
            print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step: Received first batch (idx 0).")
            print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step: Batch type: {type(batch)}")
            print(f"DEBUG_MTW: Epoch {self.current_epoch} - validation_step: slum_prediction inputs shape={inputs.shape}, labels shape={labels.shape}, paths len={len(paths)}")
        # debug end
        labels = labels.to(dtype=inputs.dtype)
        batch_size_1 = len(paths)
        ...
        if self.classifier:
            ...
            domain_labels = torch.cat([torch.zeros([batch_size_1,1]), torch.ones([batch_size_2,1])])
            domain_labels = domain_labels.to(dtype=images.dtype).to(self.device)
            outputs_, logits = self.model(torch.cat((inputs, images), axis=0))
            domain_labels = domain_labels.view(logits.shape)
        else:
        ...
        pbar['loss'] = loss
        val_stats =  { 'loss':{'val':loss}, 'accuracy':{'val':pbar['accuracy']},
                       'f1-score':{'val':pbar['f1-score']}, 'iou':{'val':pbar['iou']}, 
                       'confusion-matrix':{'val':pbar['confusion-matrix']},
                    }
        if self.classifier:
            pbar['domain_loss'] = classification_loss
            pbar['domain_accuracy'] = binary_accuracy( preds=(torch.sigmoid(logits.view((logits.shape[0],-1))) > 0.5).float(), 
                                                target=(domain_labels.view((logits.shape[0],-1))>0.5).long())
            domain_stats = {'domain_loss':{'val':classification_loss}, 'domain_accuracy':{'val':pbar['domain_accuracy']}}
            val_stats.update(domain_stats)
            loss +=  classification_loss

        return {'loss':loss, 'log':val_stats, 'progress_bar':val_stats}
...
class InspectPredictionsCallback(Callback):
    ...
    def on_validation_batch_end(self, trainer, model, outputs, batch, batch_idx, dataloader_idx): # it was on train_batch_end but then gradients would be calculated
        if (model.current_epoch % self.save_every  == 0) and (model.current_epoch > 0) and (batch_idx in self.n_batches):
            ...
            if not hasattr(self, 'base_path'):
                print("WARNING: base_path not set in InspectPredictionsCallback. Calling get_create_base_path.")
                self.get_create_base_path(trainer)            
            self.current_epoch = format(model.current_epoch, '05d')

            imgs, lbls, names = batch['slum_prediction']

            if model.classifier:
                y_hat = model(imgs)[0] > model.threshold
            else:
                y_hat = model(imgs) > model.threshold
            
            y_hat = torch.squeeze(y_hat,dim=1).detach().cpu().numpy() * 255
            lbls_ = torch.squeeze(lbls, dim=1).detach().cpu().numpy() * 255
            out_y_hat = np.stack([y_hat, np.zeros_like(y_hat), np.zeros_like(y_hat)],axis=3)
            out_lbls = np.stack([np.zeros_like(lbls_), lbls_, np.zeros_like(lbls_)],axis=3)
            out_imgs = np.transpose(copy.deepcopy(imgs.detach().cpu().numpy()), (0,2,3,1)) 
            out_imgs = (((out_imgs * self.std) + self.mean) * 255).astype(np.uint8) # Ensure correct shape for mean/std
        ...
        elif model.current_epoch == 0 and self.save_every == 1 and (batch_idx in self.n_batches):
            # Special save for epoch 0 if save_every is 1
            print(f"DEBUG: InspectPredictionsCallback.on_validation_batch_end: Saving batch {batch_idx} at epoch 0 (special save).")
            if not hasattr(self, 'base_path'):
                self.get_create_base_path(trainer)            
            self.current_epoch = format(model.current_epoch, '05d')
            ...
            out_lbls = np.stack([np.zeros_like(lbls_), lbls_, np.zeros_like(lbls_)],axis=3)
            out_imgs = np.transpose(copy.deepcopy(imgs.detach().cpu().numpy()), (0,2,3,1)) 
            out_imgs = (((out_imgs * self.std) + self.mean) * 255).astype(np.uint8)
            combined = np.concatenate([out_imgs, out_lbls, out_y_hat], axis=2)
            img_iterator = [combined[i].astype(np.uint8) for i in range(combined.shape[0])]
            self.save_batch_images(img_iterator, names)
